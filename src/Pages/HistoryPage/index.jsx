    ...
    render()
    {
        ...
        return(
            <div>
                ...
                <div className='margin-top-16 futura-22-900 center'>
                History
                </div>
                <div>
                {   this.state.data.length !== 0 ?
                    this.state.data && this.state.data.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp) ).map(item  => {
                        return(
                            <div key={item.id} className='history-card margin-top-8'>
                            {/* <Link to={{ pathname: `history/edit/${item.id}`, data: {item}}}><span style={{ float: 'right' }}>EDIT</span></Link> */}
                                <div className='futura-22-900'>{item.program_name}</div>

                    <div className='margin-top-16 futura-16-900'>
                        Session Details:
                    </div>
                    {item.playlist && item.playlist.map((item, index) => {
                       return(
                        ...
                        <div
                        className="flex-1 futura-14-600 sets-title"
                        >
                        {item.title}
                        <br />
                    ...
                    {item.selected_users && item.selected_users.map((item, index) => {
                       return(
                    ...
                    <div className='margin-top-16 center'>
                        <button className='button-delete' onClick={() => this.deleteId(item.id)}>DELETE</button>
                    </div>
                </div>
                )
                })
                : <div className='center margin-top-32 futura-14-300 gray'>No history</div>}
                </div>
