...
import REMOVE from "../HomePage/assets/icons/remove.png";
...
class EditHistory extends Component {
  constructor(props) {
    super(props);
    const initialMediaList = JSON.parse(localStorage.edit_media_list || "[]");
    const initialSessionList = JSON.parse(localStorage.edit_session_list || "[]");
    const initialUsersList = JSON.parse(localStorage.edit_users_list || "[]");
    const initialProgramName = localStorage.edit_programName || "";
    ...
    this.state = {
      ...
      redirect: false,
      selected_users: initialUsersList,
      users: [],
      titles: [],
      showExercise: false,
  ...
  componentDidMount() {
    if (this.props.playlist) {
      localStorage.setItem(
        "edit_session_list",
        JSON.stringify(
          this.props.playlist[this.props.props.match.params.id].playlist
        )
      );
      localStorage.setItem(
        "edit_programName",
        this.props.playlist[this.props.props.match.params.id].program_name
      );
      localStorage.setItem(
        "edit_users_list",
        JSON.stringify(
          this.props.playlist[this.props.props.match.params.id].selected_users
        )
      );
      this.setState({
        session_list: this.props.playlist[this.props.props.match.params.id]
          .playlist,
        selected_users: this.props.playlist[this.props.props.match.params.id]
          .selected_users,
        programName: this.props.playlist[this.props.props.match.params.id]
          .program_name,
      });
    }
  }
  ...
  handleSubmit = async (event) => {
    ...
    if(result === true){
      ...
      }).then((res) => res.json(), (window.location.href = "/history"));
    } else {
  ...
  handleSubmitMediaData = () => {
    ...
    localStorage.setItem("edit_session_list", JSON.stringify(session_list));
      ...
      <div className="video-screen">
        {this.state.addSession ? 
          <div className="exit-button">
            <img src={EXIT} onClick={() => { this.setState({ addSession: false }) }} width="20px"></img>
        </div>
        : <div className="exit-button">
          <Link to="/history">
            <img src={EXIT} width="20px"></img>
          </Link>
        </div>}
        ...
        {this.state.addSession ? 
          ...
          {this.state.selected_users &&
            this.state.selected_users.map((user, i) => (
              <div className="users-container" key={i}>
                  ...
                  <div className="flex-row">
                    <div className="flex-1 margin-auto padding-left-8">
                      <img
                        ...
                        src={REMOVE}
                        onClick={() => this.removeUser(user, i)}
                      />
                    </div>
                  </div>
                </div>
