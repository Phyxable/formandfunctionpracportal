    ...
    if (e !== "") {
      ...
      newList = oldList.filter((users) => {
        return Object.keys(users).some((key) =>
          users.email !== null
            ? users.email.toLowerCase().includes(this.state.word)
            : users
        );
      });
    ...
    return (
      <div className="container">
                ...
                <div className="flex-row">
                  <div className="flex-1 center margin-top-2">
                    <img
                      ...
                      onClick={() => this.removeItem(user, i)}
                    />
                  </div>
                </div>
              </div>
        ...
        {this.state.filterDisplay.length !== 0 ? (
          this.state.filterDisplay.map((users, j) => (
            <div className="users-container" key={j}>
              <div className="flex-row">
                ...
                <div className="flex-1 futura-14-900 margin-auto">
                  {users.name}
                  <br />
                  <div className="futura-12-300">{users.email}</div>
                </div>
                <div className="flex-row">
                  <div className="flex-1 center margin-top-2">
                    <img
          ...
          <div className="center gray futura-14-300 margin-top-16">
            No users found
          </div>
        )}
