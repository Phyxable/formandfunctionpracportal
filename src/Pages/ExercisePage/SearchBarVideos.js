        ...
        if(e !== ''){
            ...
            newList = oldList.filter(titles => {
                return Object.keys(titles).some(key =>
                  titles.title !== null ? titles.title.toLowerCase().includes(this.state.word) : titles
                );
              }
            );
    ...
    render(){
        ...
        return(
            ...
            <div className="exit-button"> 
              <img src={EXIT} onClick={() => { this.props.handleCloseVideoTab() }}
              width="20px">
              </img>
          </div>
          <div>
                <div className='container'>
                    <input placeholder='Search Videos...' onChange={e => this.handleChange(e.target.value)} />
                </div>
                {this.state.filterDisplay && this.state.filterDisplay.map((titles, j) => (
