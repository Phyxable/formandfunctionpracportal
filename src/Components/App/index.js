...
jwApi.videos.tags.list({ result_limit:1000, order_by: 'videos:desc' })
.then(response => 
  videosCategory.push(response.tags)
  );

jwApi.videos.list({ result_limit:1000 })
.then(response => 
    ...
    firebase.database().ref("/users/").once("value")
      .then(function(snapshot) {
        snapshot.forEach(function(snapshot) {
          ...
          users.push({ name: snapshot.child("userProfile").child("name").val(), email: snapshot.child("userProfile").child("email").val() });
      });
        ...
        <Router>
          <div>
            ...
            <Route 
              ...
              render={props => (
                <EditHistory props={props} playlist={this.state.playlist}/>
              )}
              />
          </div>
        </Router>
